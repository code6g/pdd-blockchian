...
/**
 * 会员信息(phyxx).控制类 -- 实现主表的管理功能
 */

@Controller                                                                                  //控制类 -- 实现主表的管理功能标签注解
@RequestMapping("/pdd/phyxxgl")                                    //pdd系统/会员信息
public class PhyxxglController extends BaseController {

    private String prefix = "pdd/phyxxgl";                              //url:pdd系统/会员信息
    ...
    public String toListPhyxx(ModelMap mmap){                              //会员信息(phyxx).跳转到列表页面。从url:/pdd/phyxx跳转-->页面:pdd/phyxx/phyxx.html
        ...
        phyxx.setBz19("2020-08-10");
        phyxx.setZcrq(InputUitl.get_date_bz());
    ...
    public TableDataInfo querylistPhyxx(Phyxx phyxx){          //会员信息(phyxx)：根据页面的form的数据组装phyxx，构造查询条件，查询列表信息，返回restful的TableDataInfo
        startPage();                                                                                   //开始分页
        phyxx=getNewPhyxxCon(phyxx);

        //query from blockChain
        //String sJson=HyxxCCTool.queryRange("hyxx_zcrq_2020-08-11","hyxx_zcrq_2030-08-11");
        String sJson=HyxxCCTool.queryRange("hyxx_zcrq_"+phyxx.getBz19()+" 00:00:00","hyxx_zcrq_"+phyxx.getZcrq()+" 23:59:59");
    ...
    public String toAddPhyxx(ModelMap mmap)
    {
        ...
        phyxx.setZcrq(InputUitl.get_datetime_bz());
    ...
    public AjaxResult addSavePhyxx(Phyxx phyxx)                                                    //根据传入的phyxx，持久化，返回结果restful-json
    {
        ...
        //save_to_blockChain
        //String jsonStr_obj=com.alibaba.fastjson.JSONObject.toJSONString(phyxx).replace("\"params\":{},","");
        //System.out.println("==com.alibaba.fastjson.JSONObject.toJSONString:"+ jsonStr_obj);
        // HyxxCCTool.insertHyxx(phyxx.getHyid(),jsonStr_obj);
        String[] arrayValue=getArrayValue(phyxx);
        int res=HyxxCCTool.insertHyxx(arrayValue)?1:0;

        //phyxx=getNewAddPhyxx(phyxx);                                                                   //对新增后的的会员信息对象进行赋值更新操作
        //return toAjax(phyxxMapper.insertPhyxx(phyxx));

        return toAjax(res);                                               //持久化
    }

    private String[] getArrayValue(Phyxx phyxx) {
    ...
    public String toEditPhyxx(@PathVariable("hyid") String hyid, ModelMap mmap)                    //根据传入的hyid，查询数据，放入ModelMap，返回给页面
    {
        ...
        //query from blockchain
        String s1=HyxxCCTool.queryHyxxByKey(hyid);
        System.out.println("==[BlockChain-hyxx]"+s1);
        Phyxx phyxx=com.alibaba.fastjson.JSONObject.parseObject(s1).toJavaObject(Phyxx.class);
    ...
    public AjaxResult editSavePhyxx(Phyxx phyxx)                                             //根据传入的phyxx，更新持久化，返回结果restful-json
    {
        ...
        String[] arrayValue=getArrayValue(phyxx);
        int res=HyxxCCTool.updateHyxx(arrayValue)?1:0;

        //phyxxMapper.updateByPhyxx(phyxx)

        return toAjax(res);                                 //返回保存结果
    }
    ...
    public AjaxResult removeOnePhyxx(@PathVariable("hyid") String hyid)                       //根据传入的hyid，删除单条数据，返回结果restful-json
    {
        ...
        int res=HyxxCCTool.deleteHyxx(hyid)?1:0;
        //phyxxMapper.deleteByHyid(hyid)
        return toAjax(res);                                                  //删除
    }
    ...
    public AjaxResult removePhyxx(@RequestParam("ids") String ids)                                //根据传入的ids，删除多个数据，返回结果restful-json
    {
        ...
        String ids2=InputUitl.getSelectConIn(ids);


       // return toAjax(phyxxMapper.deleteByHyids(ids2));
        //delete from blockchain
        ids2=ids2.replaceAll("'","").replaceAll(",","");
        int res=HyxxCCTool.deleteHyxx(ids2)?1:0;
        //phyxxMapper.deleteByHyid(hyid)
        return toAjax(res);
    }
