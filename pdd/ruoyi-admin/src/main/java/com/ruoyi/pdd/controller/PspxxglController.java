...
import com.ruoyi.blockchain.ywtool.Str8Util;
import com.ruoyi.code6g.FileUtil;
...
@RequestMapping("/pdd/pspxxgl")                                    //pdd系统/商品信息
public class PspxxglController extends BaseController {

    private String prefix = "pdd/pspxxgl";                              //url:pdd系统/商品信息
    ...
    public TableDataInfo querylistPspxx(Pspxx pspxx){          //商品信息(pspxx)：根据页面的form的数据组装pspxx，构造查询条件，查询列表信息，返回restful的TableDataInfo
        ...
        if(pspxx.getFlmc().equals("全部")){
            sJson = SpxxCCTool.queryRange(
                    "spxx_flid_", "spxx_flidc");
        }else {
            sJson = SpxxCCTool.queryRange(
                    "spxx_flid_" +  pspxx.getFlmc() + "_", "spxx_flid_" +  (pspxx.getFlmc()  + "_Z"));
        }

        System.out.println("=="+sJson);
        List<Pspxx> list = JSON.parseArray(sJson, Pspxx.class);




//        pspxx=getNewPspxxCon(pspxx);
//
//        //修改商品信息实体，构造新的查询条件
//        List<Pspxx> list = pspxxMapper.selectListByPspxx(pspxx);                                  //根据商品信息(pspxx)的属性，构造条件，查询出商品信息list
    ...
    private List<Pspxx> getNewPspxxList(List<Pspxx> list) {                                              //对查询出来的list信息进行加工处理，以便传入到前端展现
        for(Pspxx pspxx:list){
            ...
            //pspxx.setSpms(""); //商品描述
            //pspxx.setSptp(""); //商品图片
            pspxx.setSptp( InputUitl.getFj_A_href_sptp(pspxx.getSptp()) );
            //pspxx.setSpzt(""); //商品状态
            //pspxx.setBz00(""); //备注项00
            //pspxx.setBz01(""); //备注项01
            //pspxx.setBz02(""); //备注项02
    ...
    public String toAddPspxx(ModelMap mmap)
    {
        ...
        mmap.put("pspxx", pspxx);
        ArrayList<String> spzps=FileUtil.getFiles_short_by_time("/home/ruoyi/uploadPath/spzp/");
        mmap.put("spzps", spzps);                                                                       //商品信息(pspxx)：放入ModelMap
    ...
    {
        //        pspxx=getNewAddPspxx(pspxx);

        // save_to_blockChain
        String[] arrayValue=getArrayValue(pspxx);
        int res=SpxxCCTool.insertSpxx(arrayValue)?1:0;
        return toAjax(res);

                                                                 //对新增后的的商品信息对象进行赋值更新操作
//        return toAjax(pspxxMapper.insertPspxx(pspxx));                                               //持久化
    ...
    {
        pspxx= getNewAfterEditPspxx(pspxx);

        //update to blockchain
        String[] arrayValue=getArrayValue(pspxx);
        int res=SpxxCCTool.updateSpxx(arrayValue)?1:0;
    ...
    private String[] getArrayValue(Pspxx pspxx) {
//        spid	商品编码
//        flid	分类编码
//        flmc	分类名称
//        spmc	商品名称
...
//        spzt	商品状态
    ...
    public AjaxResult removeOnePspxx(@PathVariable("spid") String spid)                       //根据传入的spid，删除单条数据，返回结果restful-json
    {
        ...
        int res=SpxxCCTool.deleteSpxx(spid)?1:0;
        return toAjax(res);
    ...
    public AjaxResult removePspxx(@RequestParam("ids") String ids)                                //根据传入的ids，删除多个数据，返回结果restful-json
    {
        ...
        int res=0;
        for(String s2:s1){
            res+=SpxxCCTool.deleteSpxx(ids2)?1:0;
        }
        return toAjax(res);
